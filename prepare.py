...
def parse_dataframes(args: argparse.Namespace) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    return (fuel_dataframe, postcode_dataframe)


def remove_qa_entries(fuel_dataframe: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    return fuel_dataframe[~qa_results] # type: ignore[reportAssignmentType]
...
def clean_fuel_prices(fuel_dataframe: pd.DataFrame) -> pd.DataFrame:
...
def clean_fuel_postcodes(fuel_dataframe: pd.DataFrame) -> pd.DataFrame:
...
def clean_fuel_lat_long(fuel_dataframe: pd.DataFrame, postcode_dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    })[["brand_name", "trading_name", "postcode", "E5", "E10", "B7S", "B7P", "B10", "HVO"]] # type: ignore[reportAssignmentType]
    ...
    return pd.merge(
        ...
        how="inner",
        on="postcode"
    )[['brand_name', 'trading_name', 'postcode', 'E5', 'E10', 'B7S', 'B7P', 'B10', 'HVO', 'latitude', 'longitude']] # type: ignore[reportAssignmentType]

def write_files(fuel_dataframe: pd.DataFrame, postcode_dataframe: pd.DataFrame):
    with open("fuel.parquet", mode="wb") as fuel_file:
        # We do it this way to ensure the file can be overwritten
        fuel_file.write(fuel_dataframe.to_parquet(path=None, index=False))
...
if __name__ == "__main__":
    args = parse_args()
    fuel_dataframe, postcode_dataframe = parse_dataframes(args)
    fuel_dataframe = remove_qa_entries(fuel_dataframe)
    fuel_dataframe = clean_fuel_postcodes(fuel_dataframe)
    fuel_dataframe = clean_fuel_prices(fuel_dataframe)
    fuel_dataframe = clean_fuel_lat_long(fuel_dataframe, postcode_dataframe)
    write_files(fuel_dataframe, postcode_dataframe)
